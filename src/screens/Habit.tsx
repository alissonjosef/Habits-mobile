...
export function Habit() {
  ...
  async function fetchHabits() {
    try {
      ...
      Alert.alert(
        "Ops",
        "Não foi possivel carregar as informações dos habitos"
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handlToggleHabit(habitId: string) {
    try {
      ...
      console.log("🚀 ~ file: Habit.tsx:75 ~ handlToggleHabit ~ error:", error);
      Alert.alert("Ops", "Não foi possível atualizar status");
    }
  }
  useEffect(() => {
    fetchHabits();
      ...
      >
          ...
          {dayInfo?.possibleHabits ? (
            dayInfo?.possibleHabits.map((habit) => (
              <Checkbox
                ...
                onPress={() => handlToggleHabit(habit.id)}
        ...
        {isDateInPast && (
          <Text className="text-white mt-10 text-center">
            Voce não pode editar habito de uma data passada.
          </Text>
        )}
      </ScrollView>
    </View>
