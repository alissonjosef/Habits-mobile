...
export function New() {
  ...
  function handleToggleWeekDays(weekDasyIndex: number) {
    if (weekDays.includes(weekDasyIndex)) {
      setWeekDays((prevState) =>
        prevState.filter((weekDay) => weekDay !== weekDasyIndex)
      );
    } else {
  ...
  async function handleCreateNewHabit() {
    try {
      ...
      console.log(error);
      Alert.alert("Ops", "Não foi possível criar o novo habio");
    }
  }
    ...
    <View className="flex-1 bg-background px-8 pt-16 ">
      ...
      >
        ...
        >
          ...
          <Text className="font-semibold test-base text-white ml-2">
            Confirmar
          </Text>
        </TouchableOpacity>
      </ScrollView>
